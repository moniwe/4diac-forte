...
namespace forte::eclipse4diac::io::wago {
  ...
  FORTE_Wago636::FORTE_Wago636(const StringId paInstanceNameId, CFBContainer &paContainer) :
  ...
  void FORTE_Wago636::writeOutputData(const TEventID paEIID) {
    switch(paEIID) {
      case scmEventMAPOID: {
        writeData(17, var_QO, conn_QO);
        break;
      }
      case scmEventINDID: {
        writeData(17, var_QO, conn_QO);
        writeData(18, var_STATUS, conn_STATUS);
        break;
      }
      default:
        break;
  ...
  forte::IPlugPin *FORTE_Wago636::getPlugPinUnchecked(const size_t paIndex) {
    switch(paIndex) {
